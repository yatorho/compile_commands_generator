...
CC_PLACEHOLDER_NAME = "placeholder.cc"
CU_PLACEHOLDER_NAME = "placeholder.cu"
    ...
    for root, dirs, files in os.walk(proj):
        for fname in files:
            ...
            if any(fnmatch.fnmatch(relpath, pattern) for pattern in ignore_formats):
                if fname != CC_PLACEHOLDER_NAME and fname != CU_PLACEHOLDER_NAME:
                    continue
...
def generate_empty_cc_file(path: Path) -> None:
    ...
    if not path.exists():
        ...
        with open(path / CU_PLACEHOLDER_NAME, "w", encoding="utf-8") as f:
            f.write(code)
        print(f"✅ Created empty CUDA file at: {path / CU_PLACEHOLDER_NAME}")

        # with open(path / CC_PLACEHOLDER_NAME, "w", encoding="utf-8") as f:
        #     f.write(code)
        # print(f"✅ Created empty C++ file at: {path / CC_PLACEHOLDER_NAME}")
...
def main():
    ...
    if args.list_history:
        ...
        if args.reuse:
            ...
            if argv is None:
                raise ValueError(f"No history found for project: [{project_root}].")
            ...
            if args.root is None:
                raise ValueError("Please specify the project root with --root or -r.")

            save_last_command(args.root, sys.argv)
            generate(args)
